...
We simulate multistate mark-recapture Arnason-Schwarz [@arnason1972; @arnason1973; @schwarz1993; @pradel2005] data for animals with two alive states using R 4.2.0 [@rcoreteam2022]. We generate two types of capture histories: (1) robust design, with multiple secondary surveys during primary occasions of assumed closure [@pollock1982], and (2) "traditional", consisting of a series of single surveys. We use Bayesian inference and Markov chain Monte Carlo (MCMC) methods using NIMBLE 0.12.2 [@devalpine2017] to analyze each capture history. We then examine the traceplots and effective sample sizes of each model, demonstrating that estimation of robust design formulations is superior to traditional designs.
...
We see that mixing is much better in the robust design formulation, even with only `r n.sec` secondary surveys, and there appears to be considerably less correlation between parameters compared to the single survey model. The percent overlap between the prior and posterior distributions (PPO) is greater in the traditional model---particularly in the recapture and state transition probabilities---versus the robust design model, indicating reduced parameter identifiability in the traditional model [@gimenez2009]. Specifically, PPO of 35% is demonstrative of weak parameter identifiability. The red horizontal lines in the density plots are the prior distributions, and the red dashed lines mark the simulation input parameter values. The traditional model also takes longer to run with equal survey effort.

```{r}
#| output: true

# Prior for parameters for PPO
prior <- rbeta(niter, 1, 1)
...
The effective sample sizes of parameters are higher in the robust design formulation.
...
# Combine summaries
MSsum <- tibble(rbind(rdMSsum, tMSsum))

# Axis limits
ax.break <- round(max(MSsum$n.eff / 100) / 4) * 100
ax.lim <- floor(max(MSsum$n.eff) / ax.break) * ax.break

# Plot
MSsum %>%
  mutate(parameter = factor(rep(monitors, 2), levels = monitors),
         model = factor(c(rep("Robust design", 6), rep("Traditional", 6)))) %>%
  ggplot(aes(x = parameter)) +
  geom_col(aes(y = n.eff, fill = model),
           alpha = 3/4,
           position = position_dodge()) +
  scale_y_continuous(breaks = seq(ax.break, ax.lim, ax.break),
                     limits = c(0, max(MSsum$n.eff) + 100),
                     expand = c(0, 0)) +
  scale_fill_manual(values = c("#a4260f", "grey50")) +
  labs(title = paste0("Effective sample sizes after ", nchains * niter, " MCMC samples"),
       subtitle = "Robust design (red) vs. single survey (grey) multistate Arnason-Schwarz model",
       x = "Parameter",
