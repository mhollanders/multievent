...
We see that parameters mix well and that convergence is achieved quickly [@youngflesh2018mcmcvis]. Note that for `MCMCsummary`, we omitted the first `r n.burnin` MCMC samples as burn-in. The red dashed lines mark the simulation input parameter values.

```{r}
#| output: true

# Parameters
params <- c(pi, phi1, phi.alpha, phi.beta, psi12, psi21, p1, p2, r.delta, delta21,
            r.lambda, lambda21, mu, mu.sigma)

# Traceplots and summary
library(MCMCvis)
MEsum <- MCMCsummary(lapply(MEsamples, function(x) x[-(1:n.burnin),]),
                     params = MEmons, HPD = T) |> print()
MCMCtrace(MEsamples, params = MEmons, excl = c("b", "prev"), Rhat = T, n.eff = T, ind = T,
          pdf = F, gvals = c(params, Nsuper))
```
...
# Parameter names
names <- c("pi", "phi[1]", "alpha[phi]", "beta[phi]", "psi[12]", "psi[21]", "italic(p)[1]", "italic(p)[2]", "italic(r)[delta]", "delta[21]", "italic(r)[lambda]", "lambda[21]", "mu", "sigma[mu]", "sigma[italic(m)]", "sigma[italic(n)]")

# Plot
MCMCsummary(lapply(MEsamples, function(x) x[-c(1:n.burnin),]),
            round = 3, params = MEmons, func = median, HPD = T, 
            excl = c("b", "prev", "Nsuper")) |>
  mutate(parameter = factor(names, levels = names),
         sim = params) |>
  ...
  labs(title = "Parameter estimates",
       subtitle = "Posteriors (median and 95% HPDI, black) with input (red)",
       x = "Parameter",
       y = "Estimate") +
  coord_flip() +
